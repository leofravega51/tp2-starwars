...
const AgregarPersonaje = () => {
    ...
    const sweetSucces = (pers) => {
        Swal.fire(
            'Excelente!',
            `Personaje ${pers} agregado con éxito!`,
            'success'
        )
    }
    ...
    const sweetError = () => {
        Swal.fire({
            icon: 'error',
            title: 'Error!',
            text: 'El personaje ya existe!'
          })
    }

    const agregarPersonaje = async pers  => {
        const response = await axios.get(`${basePathApi}/agregarPersonaje?episodio=${episodio}&personaje=${pers}`);
            ...
            <Fragment>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="col-md-4" style={{position: "absolute", marginLeft:"42%", marginTop:"15%"}}>
                        ...
                        <div className="row">
                            <button type="submit" className="btn btn-secondary btn-md">Agregar</button>
                        </div>
                    </div>
                </form>
            </Fragment>
